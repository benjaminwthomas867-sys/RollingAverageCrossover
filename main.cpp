...
//placing returned data in vector
    for(auto&& [date,fiftyday,twohundredday] : std::views::zip(std::ranges::subrange(listofdates.begin()+higher,listofdates.begin()+listofdates.size()),intermediate2,intermediate1)){
        averagedata.emplace_back(date,fiftyday,twohundredday);
    }
